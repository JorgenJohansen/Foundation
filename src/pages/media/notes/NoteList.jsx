...
export default function NoteList({notes}) {
    ...
    const sendToDelete = (id) => {
        navigate(`/notat/${id}/slett`);
    }
    const sendToEdit = (id) => {
        navigate(`/notat/${id}/rediger`);
    }
        ...
        {notes?.map(note => (
            ...
            <Card sx={{width: "400px", border: "3px solid #1769aa"}}>
                ...
                action={
                    ...
                    <IconButton onClick={() => sendToEdit(note.id)}>
                        <EditOutlined />
                    </IconButton>
                    </Tooltip>
                    <Tooltip title={<Typography fontSize={15}>Slett Todo</Typography>} placement="top">
                    <IconButton onClick={() => sendToDelete(note.id)}>
                    <DeleteOutlined />
                    </IconButton>
                    </Tooltip>
                    </>
